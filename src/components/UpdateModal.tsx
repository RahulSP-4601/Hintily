...
const UpdateModal: React.FC<UpdateModalProps> = ({
    ...
    const handleCopyCommand = () => {
        navigator.clipboard.writeText('xattr -cr /Applications/Hintily.app');
        setCopied(true);
        setTimeout(() => setCopied(false), 2000);
    };
                        ...
                        {status === 'error' ? (
                                    ...
                                    {isMac ? (
                                        <>
                                            <div className="space-y-1 w-full">
                                                ...
                                                <CopyBlock command={`xattr -cr ~/Downloads/Hintily-${displayVersion.replace('v', '')}-${instructionsArch || 'arm64'}.dmg`} />
                                            </div>
                                            <div className="space-y-1 mt-1 pl-0.5">
                                                <p className="text-[12px] font-medium text-white/80">{t('2. Open the file and install Hintily.')}</p>
                                            </div>
                                            <div className="space-y-1 mt-3 w-full">
                                                ...
                                                <CopyBlock command="xattr -cr /Applications/Hintily.app" />
                                            </div>
                                        </>
                                    ) : (
                                        <div className="space-y-1 w-full">
                                            <p className="text-[12px] font-medium text-white/80">{t('Run the downloaded installer (.exe) and follow the prompts. Hintily will restart when finished.')}</p>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="p-8 flex flex-col items-center justify-center h-full text-center relative">
                                ...
                                {/* 2. Premium Troubleshooting Card — macOS-only.
                                    ...
                                    equivalent), and the /Applications/Hintily.app
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between bg-black/20 rounded-lg pl-3 pr-1.5 py-1.5 border border-white/[0.03] group hover:border-white/10 transition-colors">
                                        <code className="text-[10px] font-mono text-blue-400 truncate mr-2 select-all">
                                            xattr -cr /Applications/Hintily.app
                                        </code>
                                        <button
