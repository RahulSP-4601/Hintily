...
test('provider readiness precedes allocation activation', () => {
  ...
  assert.match(managedAi, /action === 'ready'[\s\S]*checkProviderReadiness\(provider\)/);
  ...
  assert.match(managedAi, /requested_claim_id: readinessClaimId/);
  ...
  assert.match(managed, /channelStartupFailed[\s\S]*this\.activated[\s\S]*this\.stop\('stt_connection_failed'\)/);
